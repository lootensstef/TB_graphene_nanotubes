...
def test_graphene_band_arrays_shape():
    ...
    k_band, e_p_band, e_m_band = eb_simulation.simulate_energybands(
        "graphene", None, None, N, 0.0, -2.75, 0.05)
...
def test_nanotube_band_arrays_valid_output():
    ...
    k_band, e_p_band, e_m_band = eb_simulation.simulate_energybands(
        "nanotube", 12, 13, N, 0.0, -2.75, 0.05)
...
def test_reasonable_zigzag_nanotubes_bandgap():
    ...
    for n in n_zigzag:
        _, e_p_bands, e_m_bands = eb_simulation.simulate_energybands(
            "nanotube", n, m_zigzag, 1000, 0, -2.75, 0.05)
...
def test_reasonable_armchair_nanotubes_bandgap():
    ...
    for n_m in n_m_armchair:
        _, e_p_bands, e_m_bands = eb_simulation.simulate_energybands(
            "nanotube", n_m, n_m, 1000, 0, -2.75, 0.05)
        assert np.isclose((np.min(e_p_bands) - np.max(e_m_bands)), 0)
...
def test_reasonable_semiconducting_nanotubes_bandgap():
    ...
    for n_m in n_m_armchair:
        _, e_p_bands, e_m_bands = eb_simulation.simulate_energybands(
            "nanotube", n_m, n_m - 1, 1000, 0, -2.75, 0.05)
