...
def graphene_symmetrylines(N):
    ...
    Parameters:
        N (integer): Number of points for each line. Affects calculated bands smoothness and calculation speeds.

    Returns:
        m_to_gamma, gamma_to_k, k_to_m (2,N arrays): the segmented symmetry lines in reciprocal space, the names are self explanatory"""

    m_to_gamma = np.array([np.linspace(1 / 2, 0, N), np.zeros(N)])
    gamma_to_k = np.array([np.linspace(0, 2 / 3, N), np.linspace(0, 1 / 3, N)])
    k_to_m = np.array([np.linspace(2 / 3, 1 / 2, N),
                      np.linspace(1 / 3, 1 / 2, N)])
