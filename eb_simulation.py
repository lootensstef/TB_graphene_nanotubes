...
def simulate_energybands(
    ...
    if input_material == "graphene":
        ...
        m_to_gamma, gamma_to_k, k_to_m = graphene_space.graphene_symmetrylines(
            input_precision)

        bandfunc_m_gamma = tb_simple_nn.bandfunc(
            m_to_gamma[0, :], m_to_gamma[1, :])
        bandfunc_gamma_k = tb_simple_nn.bandfunc(
            gamma_to_k[0, :], gamma_to_k[1, :])
        bandfunc_k_m = tb_simple_nn.bandfunc(k_to_m[0, :], k_to_m[1, :])

        e_p_mg, e_m_mg = tb_simple_nn.eigenvals(
            bandfunc_m_gamma, input_eps2p, input_gamma_0, input_s_0)
        e_p_gk, e_m_gk = tb_simple_nn.eigenvals(
            bandfunc_gamma_k, input_eps2p, input_gamma_0, input_s_0)
        e_p_km, e_m_km = tb_simple_nn.eigenvals(
            bandfunc_k_m, input_eps2p, input_gamma_0, input_s_0)
    ...
    if input_material == "nanotube":
        ...
        for j in j_values:
            ...
            x_to_x = nanotube_space.nanotube_symmetryline(
                k_gamma, k_z, j, input_precision)

            bandfunc_x_x = tb_simple_nn.bandfunc(x_to_x[0, :], x_to_x[1, :])

            e_p_x_x, e_m_x_x = tb_simple_nn.eigenvals(
                bandfunc_x_x, input_eps2p, input_gamma_0, input_s_0)
